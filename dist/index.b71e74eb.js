...
user.eventing.on("click", ()=>{
    console.log(`Hey Galien!\ni am changed`);
});
user.eventing.trigger('click') // user.set({name:"Muhindo",age:23})
 // user.save()
 // user.fetch()
 // setTimeout(()=>{
 //   console.log(user);
...
class User {
    constructor(data){
        this.data = data;
        this.eventing = new _eventing.Eventing();
...
},{"./Eventing":"7459s","./Sync":"QO3Gl","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7459s":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Sync {
    fetch(id) {
        _axiosDefault.default.get(`http://localhost:3000/users/${id}`);
    }
    save(data) {
        const { id  } = data;
        if (id) _axiosDefault.default.put(`http://localhost:3000/users/${id}`, data);
        else _axiosDefault.default.post('http://localhost:3000/users', data);
    }
}
...
},{"./../utils":"5By4s"}]},["8wcER","h7u1C"], "h7u1C", "parcelRequire94c2")
